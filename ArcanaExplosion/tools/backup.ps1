...
#   _backups\ArcanaExplosion_20260902-2249.zip  みたいなファイルが1個できる。
...
$zipPath   = Join-Path $backupDir "ArcanaExplosion_$stamp.zip"
...
$tmp = Join-Path $env:TEMP "ArcanaExplosion_backup_$stamp"
...
# 古いバックアップを整理
$all = Get-ChildItem $backupDir -Filter "ArcanaExplosion_*.zip" | Sort-Object Name -Descending
...
Get-ChildItem $backupDir -Filter "ArcanaExplosion_*.zip" |
